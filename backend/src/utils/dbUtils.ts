...
import { StudySpaceType, FoodRetailerType } from '../models/FacilityModel2'
...
export async function createStudySpaceSummary(studySpace: StudySpaceType): Promise<string> {
  return new Promise((resolve) => {
    const basicInfo = `${studySpace.name}, located at ${studySpace.location}`;
    const campus = studySpace.campus ? `Campus: ${studySpace.campus}` : "Campus: Not specified";
    const description = `Description: ${studySpace.description}`;
    const keywords = studySpace.key_words ? `Keywords: ${studySpace.key_words.join(", ")}` : "No keywords provided";
    ...
    const amenities = studySpace.amenities ? `Amenities: ${studySpace.amenities.join(", ")}` : "No amenities listed";
    const additionalInfo = studySpace.additional_info ? `Additional Info: ${JSON.stringify(studySpace.additional_info)}` : "No additional information";
...
export async function createFoodRetailerSummary(foodRetailer: FoodRetailerType): Promise<string> {
  return new Promise((resolve) => {
    const basicInfo = `${foodRetailer.name}, located at ${foodRetailer.location}`;
    const campus = foodRetailer.campus ? `Campus: ${foodRetailer.campus}` : "Campus: Not specified";
    const description = `Description: ${foodRetailer.description}`;
    const keywords = foodRetailer.key_words ? `Keywords: ${foodRetailer.key_words.join(", ")}` : "No keywords provided";
    ...
    const urls = foodRetailer.urls ? `URLs: ${foodRetailer.urls.join(", ")}` : "No URLs provided";
    const additionalInfo = foodRetailer.additional_info ? `Additional Info: ${JSON.stringify(foodRetailer.additional_info)}` : "No additional information";
...
const facilitySummaryFunctions: Record<string, (record: any) => Promise<string>> = {
...
export class XenovaEmbeddings implements EmbeddingsInterface {
